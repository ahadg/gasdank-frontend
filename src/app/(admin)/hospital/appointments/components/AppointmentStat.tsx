    ...
    <Card>
      <CardBody>
        <div className="d-flex align-items-start gap-3">
          ...
          <div>
            <CardTitle as={'h4'} className="mb-1 d-flex align-items-center gap-2">{title}</CardTitle>
            <p className="text-primary fw-medium fs-20 mb-0">{count} <span className="fs-15 text-muted ms-1"> Today</span></p>
          </div>
        ...
        <div className="mt-3">
          {
            patients.map((item, idx) => (
              <p key={idx} className="text-dark mb-1 fs-14 d-flex justify-content-between">{item.patients} <span className="text-dark fw-semibold">{item.patientsCount} Patients</span></p>
            ))
          }
        </div>
      </CardBody>
      <CardFooter className="border-top border-dashed">
        <Link href="" className="link-primary fw-medium">View More <IconifyIcon icon="tabler:arrow-right" /></Link>
      </CardFooter>
    </Card>
  )
}
