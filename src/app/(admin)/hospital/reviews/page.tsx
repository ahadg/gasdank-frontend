  ...
  return (
    <Card className="overflow-hidden">
      <CardHeader className="d-flex align-items-center gap-2 border-bottom border-dashed bg-light-subtle">
        <Image src={doctors.image} alt='' className="avatar-xl rounded-circle border border-light border-2" />
        <div>
      ...
      <CardBody>
            ...
            <Fragment key={idx}>
              <div className={`${idx != 1 ? 'mt-3' : 'mb-3'} `}>
                <div className="d-flex justify-content-between">
                  ...
                  <div>
                    <p className="mb-0 fs-13 fw-medium text-muted">{item.date.toLocaleString('en-us', { day: '2-digit', month: 'short', year: 'numeric' })}</p>
                  </div>
                </div>
                <p className="my-2">{item.description}</p>
                <div>
                  <Link href="" className="fs-13 me-2 link-reset icons-center"><IconifyIcon icon='tabler:thumb-up' className="fs-16 me-1" /> {item.like}</Link>
                  <Link href="" className="fs-13 me-3 link-reset icons-center"><IconifyIcon icon='tabler:thumb-down' className="fs-16 me-1" /> {item.dislike}</Link>
                </div>
              </div>
             {idx != 1 && <hr />} 
            </Fragment>
        ...
        <Link href="" className="link-primary fw-medium">View More Review</Link>
      </CardBody>
    </Card>
  )
}
