  ...
  return (
    <Card>
      <CardHeader className="border-bottom border-dashed">
        <CardTitle as={'h4'} className="mb-0">Treatment History </CardTitle>
      </CardHeader>
          ...
          <tbody>
            {
              treatmentData.map((item, idx) => (
                <tr key={idx}>
                  ...
                  <td>
                    <span className="badge bg-light text-dark p-1 fs-11"><IconifyIcon icon='tabler:check' className="text-success" /> Wll Done</span>
                  </td>
                  <td><span className={`badge bg-${item.paymentStatus == 'Pending' ? 'warning' : 'success'} p-1 fs-11`}>{item.paymentStatus}</span></td>
                  <td className="pe-3">
