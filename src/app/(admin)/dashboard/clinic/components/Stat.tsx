...
const StatCard = ({ count, details, icon, title, isLabel, isDot }: StatType) => {
  return (
    <Card>
      <CardBody>
        <Link href="" className="text-muted float-end mt-n1 fs-18"><IconifyIcon icon="tabler:external-link" /></Link>
        ...
        <div className="d-flex align-items-center gap-2 my-3">
          ...
          <h3 className="mb-0 fw-bold">{count} {isLabel && <span className="badge text-bg-success fw-medium ms-2 fs-12">{isLabel}</span>}</h3>
        </div>
        {
          details.map((data, idx) => (
            <p className="mb-1" key={idx}>
              <span className="text-primary me-1"> {isDot ? <IconifyIcon icon="tabler:point-filled" /> : <IconifyIcon icon="tabler:minus" />}  </span>
              <span className="text-nowrap text-muted">{data.title}</span>
              <span className="float-end"><b>{data.count}</b></span>
            </p>
          ))
        }
      </CardBody>
