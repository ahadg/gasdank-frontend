  ...
  return (
    <Card>
      <CardHeader className="d-flex justify-content-between align-items-center">
        ...
        <Button variant='secondary' size='sm'>Add New <IconifyIcon icon='tabler:plus' className="ms-1" /></Button>
      </CardHeader>
            ...
            <tbody>
              {
                appointmentData?.map((item, idx) => (
                  <tr key={idx}>
                    <td>#{idx}</td>
                    <td><Link href="" className="link-reset fw-medium">{item.name}</Link></td>
                    <td>{item.gender}</td>
                    <td>{item.age}</td>
                    <td>
                      {item.appointment}
                    </td>
                    <td>{item.date.toLocaleString('en-us', { day: '2-digit', month: 'short', year: 'numeric' })} <small className="text-muted">{item.date.toLocaleString('en-us', { hour: '2-digit', minute: '2-digit' })}</small></td>
                    <td> {item.doctors.image && <Image src={item.doctors.image} alt='doctors' className="avatar-xs rounded-circle me-1" />}
                      <Link href="" className="link-reset fw-medium">{item.doctors.name}</Link>
                    </td>
                    <td><span className={`badge bg-${item.appointmentStatus == 'Canceled' ? 'danger' : item.appointmentStatus == 'Scheduled' ? 'warning' : 'success'}-subtle text-${item.appointmentStatus == 'Canceled' ? 'danger' : item.appointmentStatus == 'Scheduled' ? 'warning' : 'success'} p-1`}>{item.appointmentStatus}</span></td>
                    <td>
