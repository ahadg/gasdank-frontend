              ...
              <Row className="justify-content-between g-3">
                ...
                <Col lg={6}>
                  <div className="text-md-end mt-3 mt-md-0">
                    <button type="button" className="btn btn-success waves-effect waves-light me-1"><IconifyIcon icon='tabler:settings' className="me-1" />More Setting</button>
                    <button type="button" className="btn btn-dark waves-effect waves-light"><IconifyIcon icon='tabler:filter' className="me-1" /> Filters</button>
                  </div>
                </Col>
              </Row>
            </CardHeader>
                ...
                <tbody>
                  {
                    orderData.map((item, idx) => (
                      <tr key={idx}>
                        ...
                        <td>
                          <Link href="/e-commerce/order-details" className="text-muted fw-medium">#BM9708</Link>
                        </td>
                        <td>
                          <h5 className="mb-0 text-dark">{item.users?.image && <Image src={item.users?.image} alt='avatar' className="rounded-circle avatar-xs me-1" />} <Link href="" className="text-dark">{item.users?.name}</Link></h5>
                        </td>
                        <td>
                          <p className="mb-1"><span className="text-dark fw-semibold">P1 -</span> {item.product.p1}</p>
                          {
                            item.product.p2 &&
                            <p className="mb-0"><span className="text-dark fw-semibold">P2 -</span> {item.product.p2}</p>
                          }
                        </td>
                        <td>
                          ...
                          {
                            item.quantity.p2 &&
                            <p className="mb-0">{item.quantity.p2} Piece</p>
                          }
                        </td>
                        <td>
                          ${item.total}
                        ...
                        <td>
                          <h5 className="mb-0"><span className={`badge text-${item.paymentStatus == 'Failed' ? 'danger' : item.paymentStatus == 'Pending' ? 'warning' : 'success'} border border-${item.paymentStatus == 'Failed' ? 'danger' : item.paymentStatus == 'Pending' ? 'warning' : 'success'}-subtle fs-11 p-1`}>{item.paymentStatus}</span>
                          </h5>
                        </td>
                        <td>
                          <h5 className="mb-0"><span className={`badge badge-soft-${item.orderStatus == 'Delivered' ? 'info' : item.orderStatus == 'Dispatched' ? 'warning' : item.orderStatus == 'Ready To Pick' ? 'dark' : 'danger'} fs-11 p-1`}>
                            {
