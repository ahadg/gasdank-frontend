...
const WizardButtons = ({ onPrev, onNext, activeStep, totalSteps, onFinish, goToFirstStep }: any) => (
  <div className="d-flex wizard justify-content-between flex-wrap gap-2 mt-3">
    <div className="first">
      <Button onClick={goToFirstStep} disabled={activeStep === 1} className="btn btn-primary">
        First
      </Button>
    </div>
    <div className="d-flex flex-wrap gap-2">
      <div className="previous">
        <Button onClick={onPrev} disabled={activeStep === 1} className="btn btn-primary">
          <i className="bx bx-left-arrow-alt me-2" /> Back To Previous
        </Button>
      </div>
      <div className="next">
        <Button onClick={onNext} disabled={activeStep === totalSteps} className="btn btn-primary mt-3 mt-md-0">
          Next Step <i className="bx bx-right-arrow-alt ms-2" />
        </Button>
      </div>
    </div>
    <div className="last">
      <Button onClick={onFinish} disabled={activeStep === totalSteps} className="btn btn-primary mt-3 mt-md-0">
        Finish
      </Button>
    </div>
  </div>
...
const BasicWizard = () => {
  ...
  const steps = [
    ...
    {
      title: 'Profile',
      icon: 'font-18 align-middle me-1',
    },
    {
      title: 'Finish',
      icon: 'mdi mdi-checkbox-marked-circle-outline font-18 align-middle me-1',
    },
  ]
  ...
  return (
    <Card>
      <Card.Body className="overflow-hidden">
        <h4 className="header-title mb-3">A Basic Wizard</h4>

        <ul className="nav nav-pills nav-justified form-wizard-header mb-3">
          {steps.map((step, idx) => (
            <li key={idx} className="nav-item">
              <Button as={'a'} className={`nav-link rounded-0 py-2 ${activeStep === idx + 1 ? 'active' : ''}`} onClick={() => goToStep(idx + 1)}>
  ...
  return (
    <Card>
      <Card.Body className="overflow-hidden">
        ...
        <ProgressBar animated striped variant="success" style={{ height: '7px' }} now={(activeStep / 3) * 100} className="mb-3 progress-sm" />
              ...
              <Row>
                <Col sm={12}>
                  <div className="text-center">
                    ...
                    <h3>Thank you!</h3>
                    <p>Quisque nec turpis at urna dictum luctus. Suspendisse convallis dignissim eros at volutpat.</p>
                  </div>
                </Col>
                <Col sm={12}>
                  <WizardButtons
