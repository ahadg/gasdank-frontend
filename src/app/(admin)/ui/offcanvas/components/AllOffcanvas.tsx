...
const DefaultOffcanvas = () => {
  ...
  return (
    <ComponentContainerCard title="Offcanvas"
                            description={<>You can use a link with the <code>href</code> attribute, or a button with
                              the <code>data-bs-target</code> attribute. In both cases,
                              the <code>data-bs-toggle=&quot;offcanvas&quot;</code> is required.</>}>
      <div className="d-flex flex-wrap gap-2">
        <Button variant="primary" onClick={toggle} data-bs-toggle="offcanvas" href="#offcanvasExample" role="button"
                aria-controls="offcanvasExample">
          Link with href
        </Button>
      </div>
      <Offcanvas show={isTrue} onHide={toggle} className="offcanvas-start" tabIndex={-1} id="offcanvasExample"
...
const OffcanvasBackdrop = () => {
  ...
  return (
    <ComponentContainerCard title="Offcanvas Backdrop"
                            description={<>Scrolling the <code>&lt;body&gt;</code> element is disabled when an offcanvas
                              and its backdrop are visible. Use the <code>data-bs-scroll</code> attribute to
                              toggle <code>&lt;body&gt;</code> scrolling and <code>data-bs-backdrop</code> to toggle the
                              backdrop.</>}>
...
const OffcanvasPlacement = () => {
  const OffcanvasPlacement = ({ name, ...props }: PlacementOption) => {
    ...
    return (
      <>
        <Button onClick={toggle} className="mt-2 me-1 mt-md-1">
          {' '}
  ...
  return (
    <ComponentContainerCard title="Offcanvas Placement">
      <div>
...
const DarkOffcanvas = () => {
  const { isTrue, toggle } = useToggle()
  return (
    <ComponentContainerCard title="Dark Offcanvas"
                            description={<>Change the appearance of offcanvases with utilities to better match them to
                              different contexts like dark navbars.
                              Here we add <code>.text-bg-dark</code> to
                              the <code>.offcanvas</code> and <code>.btn-close-white</code> to
      ...
      <div>
        <button onClick={toggle} className="btn btn-primary mt-2 mt-md-0" type="button" data-bs-toggle="offcanvas"
                data-bs-target="#offcanvasDark" aria-controls="offcanvasDark">Dark offcanvas
        </button>
        <Offcanvas show={isTrue} onHide={toggle} className="offcanvas-start text-bg-dark" tabIndex={-1}
